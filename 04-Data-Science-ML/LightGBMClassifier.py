...
# MAGIC - Compare trials in the [MLflow experiment](#mlflow/experiments/3131765080193158).
...
input_data_path = mlflow.artifacts.download_artifacts(run_id="0aabe32db71c4d1ab04679c4609275b6", artifact_path="data", dst_path=input_temp_dir)
...
# MAGIC
# MAGIC `_automl_split_col_3914` contains the information of which set a given row belongs to.
...
# AutoML completed train - validation - test split internally and used _automl_split_col_3914 to specify the set
split_train_df = df_loaded.loc[df_loaded._automl_split_col_3914 == "train"]
split_val_df = df_loaded.loc[df_loaded._automl_split_col_3914 == "val"]
split_test_df = df_loaded.loc[df_loaded._automl_split_col_3914 == "test"]

# Separate target column from features and drop _automl_split_col_3914
X_train = split_train_df.drop([target_col, "_automl_split_col_3914"], axis=1)
y_train = split_train_df[target_col]

X_val = split_val_df.drop([target_col, "_automl_split_col_3914"], axis=1)
y_val = split_val_df[target_col]

X_test = split_test_df.drop([target_col, "_automl_split_col_3914"], axis=1)
...
# MAGIC - All the runs are logged under [this MLflow experiment](#mlflow/experiments/3131765080193158)
...
def objective(params):
  with mlflow.start_run(experiment_id="3131765080193158") as mlflow_run:
...
space = {
  "colsample_bytree": 0.6975163994163529,
  "lambda_l1": 2.2529796361459455,
  "lambda_l2": 3.25403192907117,
  "learning_rate": 1.0211367068793238,
  "max_bin": 395,
  "max_depth": 5,
  "min_child_samples": 180,
  "n_estimators": 277,
  "num_leaves": 12,
  "path_smooth": 34.520256664630345,
  "subsample": 0.5632635660119487,
  "random_state": 166759989,
}
...
if shap_enabled:
    ...
    train_sample = X_train.sample(n=min(100, X_train.shape[0]), random_state=166759989).fillna(mode)

    # Sample some rows from the validation set to explain. Increase the sample size for more thorough results.
    example = X_val.sample(n=min(100, X_val.shape[0]), random_state=166759989).fillna(mode)
...
print(f"%md [Link to model run page](#mlflow/experiments/3131765080193158/runs/{ mlflow_run.info.run_id }/artifactPath/model)")
